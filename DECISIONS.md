...
### [Day 1 / Block 9] Backend deploys to Render.com as a native Python Web Service, not a Hugging Face Space - 2026-08-26
Decision: the FastAPI backend is a Render Web Service with Root Directory `backend`, start command `uvicorn main:app --host 0.0.0.0 --port $PORT`, on Render's native Python 3 runtime. The frontend stays on Vercel with Root Directory `frontend`; the database stays Supabase. The root `Dockerfile`, `.dockerignore` and the README's HF YAML frontmatter are no longer part of any deploy path.
Context: Block 9 item 4 was written against HF Spaces and never pushed. Spaces with a Docker environment are a paid tier, so the plan had no free path to a running container, and `docker build` had never been executed locally either (no Docker daemon on this machine) - the first real build would have been the Space's.
Alternatives: Railway (free tier is trial credit that expires, so the demo could go dark mid-week); Koyeb (free instance exists but the monorepo path needs a Dockerfile or buildpack config rather than a directory field); AWS EC2 (a VM means we own the process supervisor, TLS and the systemd unit - hours of work for one uvicorn process, and a card on file).
Rationale: Render gives all three things this backend actually needs on the free tier - a native Python 3 runtime (so no container to build, which retires the unbuilt-Dockerfile risk outright), a Root Directory field that targets `backend/` in a monorepo directly, and GitHub integration that deploys on push to `main` with no second git remote. That last point also removes the disclosure question the Space raised: a Space is its own git repo, so pushing to it would have published `AUDIT.md`, `DIY.md` and `PRD.md` at the Space URL. Render reads the existing GitHub repo, so repo visibility stays a single decision in one place. Cost of the switch: Render's free tier spins a service down after inactivity and its first request pays a cold start, and the free instance has a 512 MB memory ceiling - which is the constraint to check before the Block 8 weight swap adds `torch`.

### [Day 2 / Lane B] Drill-down blocked on FROZEN schema changes — 2026-08-26
Decision: Stopped drill-down feature implementation (Task 2). Logged as cross-lane requests in DIY.md instead of proceeding.
Context: Drill-down requires (1) site/activity UUID in DensityRow (currently only name available) to match reports, and (2) activity filter parameter on GET /api/v1/reports. Both require changes to FROZEN files (schemas.py, routes/reports.py) that are outside Lane B's scope.
Alternatives: (1) Fetch all reports client-side and filter on frontend (inefficient, not scalable); (2) Proceed with site-only drill-down using name + lookup query (partial solution, does not fix activity); (3) Add DIY requests and halt implementation (chosen).
Rationale: FROZEN files break every lane when edited without coordination. Logging cross-lane needs in DIY.md aligns with the lane model in STAGES.md and preserves single-integrator control. These are genuine architectural needs, not workarounds, so they belong as explicit asks rather than hacks. Lane B work continues on other tasks while the integrator resolves the schema.

### [Day 2 / Lane B] Drill-down feature: Integrator override of FROZEN files for group_id + activity filter — 2026-08-26
Decision: Modified FROZEN `backend/schemas.py`, `backend/routes/reports.py`, and created frontend drill-down modal with explicit integrator authorization.
Context: The density table click-to-drill-down feature requires (1) a UUID for each site to pass to the reports query, (2) an activity filter on the reports endpoint to match activity precursor spans, and (3) frontend modal with loading/loaded/failed state. Both backend changes touch FROZEN contract files and would normally block Lane B; the integrator (Swayam) approved the modifications in writing.
Alternatives: (1) Only drill by site name (requires a second lookup query to find the UUID, and loses activity bucketing); (2) Client-side fetch-all-and-filter (inefficient, does not scale to production data); (3) Defer to a later integrator session (leaves the dashboard incomplete).
Rationale: The drill-down is a core Tier 1 feature (see the density table in PRD.md § Tier 1 depth, item 2: "Click a row to show the reports behind its score"). The integrator's explicit override is documented here and in DIY.md rather than hidden; all changes are logged by file and line number; no field naming or nullability changed from the contract, only new optional fields added (`group_id: UUID | None` and `activity: str | None`). The modifications remain read-only for any other lane.

### [Day 2 / Lane B] group_id field addition to DensityRow — 2026-08-26
Decision: Added `group_id: UUID | None` field to DensityRow class in backend/schemas.py at line 218 with docstring; updated backend/routes/analytics.py to pass site_id through rank_groups() and into DensityRow constructor; modified backend/analytics/density.py rank_groups() to preserve site_id in output dict.
Context: The frontend drill-down modal needs a UUID for each ranked site/activity pair to query `/api/v1/reports?group_id=<uuid>&activity=<activity>`. The DensityRow schema was the natural place to carry this since every row already carries group_name.
Alternatives: (1) Separate lookup query client-side (adds latency and a second endpoint hit); (2) Store site_id in a parallel cache on the frontend (loses the single source of truth); (3) Pass it in the API response body outside DensityRow (inconsistent with the schema's single row type).
Rationale: The field is optional (None for activities with no site context), carries a UUID (not guessed), and follows the discriminated-union pattern for "data present or not present" rather than fabricating a default. The choice to preserve it through rank_groups() rather than compute it fresh on each row keeps the density algorithm's isolation — rank_groups() does not need to know about sites, it just preserves what it receives.

### [Day 2 / Lane B] activity filter addition to GET /api/v1/reports — 2026-08-26
Decision: Added `activity: str | None` parameter to list_reports() in backend/routes/reports.py at lines 148-195. Filter uses `precursors.entity_text.ilike(f"{activity}%")` after a precursor!inner() join swap to support filtering by precursor content.
Context: The drill-down modal needs to filter reports by both site and activity (e.g., "show me welding incidents at Ramgarh"). The activity parameter names a precursor word, and the filter matches reports whose precursor spans start with that word (case-insensitive).
Alternatives: (1) Exact match on first word only (loses "welding-related" incidents if the precursor text varies); (2) Full-text search (Supabase REST has no FTS operator); (3) Client-side filter after fetching all reports (inefficient).
Rationale: The ilike starts-with match aligns with how density.py buckets activities (`first_word = precursor.split()[0]`). A single verb like "welding" will match "welding arc", "welding station", etc. The precursor!inner() join ensures only reports with at least one precursor are returned when activity is filtered (not a "null leaks through" footgun).

### [Day 2 / Lane B] Frontend drill-down modal with discriminated Load state — 2026-08-26
Decision: Created frontend/app/dashboard/drill_down_modal.tsx with a discriminated Load union: `{ name: "loading" } | { name: "loaded", data: Report[] } | { name: "failed", error: ApiError }`. Modal invokes the activity filter endpoint and renders three states explicitly.
Context: PATTERNS.md § 3 mandates discriminated unions for load states; the modal must handle network failures gracefully without rendering stale data or duplicate requests.
Alternatives: (1) Three boolean flags (loading, error, success — does not prevent invalid state combinations); (2) Fetch inside a useEffect with try-catch (complicates testing and reintroduces the "should I refetch" logic); (3) Hardcode a fallback or placeholder (violates the "never render invented data" rule).
Rationale: The union type forces exhaustive handling (TypeScript) and makes invalid states impossible to represent. The modal sits inside density_table.tsx and receives the group_id + activity pair on click; drill_down_modal.tsx owns the fetch logic and state machine, density_table.tsx owns the visibility toggle. Separation of concerns: one component per responsibility.

### [Day 2 / Lane B] Density table clicks updated to open drill-down modal — 2026-08-26
Decision: Modified frontend/app/dashboard/density_table.tsx to (1) track selectedRow state, (2) render rows as clickable (`cursor-pointer`), (3) pass group_id + activity to DrillDownModal on click.
Context: The table row data already carries group_id (added to DensityRow schema); clicking a row should open a modal showing the reports for that site/activity pair.
Alternatives: (1) Link to a separate /reports?site=X&activity=Y page (navigates away, loses context); (2) Inline expansion (makes the table very tall, hard to scroll); (3) No drill-down (incomplete feature).
Rationale: A modal preserves the table context and allows quick dismissal to compare rows. The onClick handler reads group_id and activity from the row; if group_id is null, the click is ignored (activity-only drill is not supported).

### [Day 2 / Lane B] Type updates to api_client.ts for drill-down support — 2026-08-26
Decision: Updated frontend/lib/api_client.ts (1) added `activity?: string` to ReportFilters interface (line ~260), (2) added `group_id: string | null` to DensityRow interface (line ~101).
Context: The frontend types must reflect the new schema fields so TypeScript verification passes and drill-down code compiles.
Alternatives: (1) Cast to `any` in the modal (silences the type checker but loses the guarantees); (2) Deferring type updates (valid only until the next `npm run build`).
Rationale: The contract lives in api_client.ts; the frontend code reads from it. Keeping types in sync prevents silent data mismatches that manifest only at runtime.
